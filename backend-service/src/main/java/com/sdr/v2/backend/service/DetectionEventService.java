...
public class DetectionEventService {
    ...
    public DetectionEventService(AppProperties props, IqMlModelService iqMlModel) {
        this.props = props;
    ...
    public synchronized void onIqFrame(RawIqFrame frame) {
        ...
        Classification staticClassification = classify(features, threshold, previousState, sampleTsNs);
        lastSpectralByDevice.put(deviceKey, new SpectralState(features.peakFreqHz(), sampleTsNs));

        IqMlModelService.MlInference mlInference = null;
        if (props.getDetection().getMl().isEnabled()) {
            mlInference = iqMlModel.infer(new IqMlModelService.FeatureVector(
                    features.avgAbsEnergy(),
                    features.powerVariance(),
                    features.zeroCrossingRate(),
                    features.spectralFlatness(),
                    features.occupiedBandwidthHz(),
                    features.peakToMeanDb(),
                    features.peakFreqHz(),
                    features.centerFreqHz(),
                    staticClassification.hopDeltaHz(),
                    staticClassification.hopRateHzPerSec(),
                    staticClassification.wifiScore(),
                    staticClassification.droneScore(),
                    staticClassification.controlLinkScore(),
                    staticClassification.digitalVideoScore(),
                    staticClassification.fhssScore()
            ));
        }
        FusionDecision fusion = fuse(staticClassification, mlInference);

        String cooldownKey = deviceKey + "/" + fusion.type();
        ...
        evidence.put("wifiScore", staticClassification.wifiScore());
        evidence.put("droneScore", staticClassification.droneScore());
        evidence.put("controlLinkScore", staticClassification.controlLinkScore());
        evidence.put("digitalVideoScore", staticClassification.digitalVideoScore());
        evidence.put("fhssScore", staticClassification.fhssScore());
        evidence.put("hopDeltaHz", staticClassification.hopDeltaHz());
        evidence.put("hopRateHzPerSec", staticClassification.hopRateHzPerSec());
        evidence.put("protocolLabel", staticClassification.type());
        evidence.put("staticLabel", staticClassification.type());
        evidence.put("staticConfidence", staticClassification.confidence());
        evidence.put("fusedLabel", fusion.type());
        evidence.put("fusedConfidence", fusion.confidence());
        evidence.put("fusionMode", fusion.mode());

        if (mlInference != null) {
            evidence.put("mlEnabled", true);
            evidence.put("mlLabel", mlInference.label());
    ...
    private FusionDecision fuse(Classification staticClassification, IqMlModelService.MlInference mlInference) {
        if (mlInference == null) {
            return new FusionDecision(
                    staticClassification.type(),
                    staticClassification.severity(),
                    clamp01(staticClassification.confidence()),
                    "static-only"
            );
        }

        AppProperties.Detection.Ml mlConfig = props.getDetection().getMl();
        double blend = clamp01(mlConfig.getBlend());
        double staticConf = clamp01(staticClassification.confidence());
        double mlConf = clamp01(mlInference.confidence());

        String staticLabel = staticClassification.type();
        String mlLabel = mlInference.label();
        if (staticLabel.equals(mlLabel)) {
            double fused = clamp01((staticConf * (1.0 - blend)) + (mlConf * blend));
            return new FusionDecision(
                    staticLabel,
                    staticClassification.severity(),
                    fused,
        ...
        boolean strongMlOverride = mlConf >= clamp01(mlConfig.getOverrideThreshold())
                && (mlInference.nearKnownChannel() || isHighPriorityMlLabel(mlLabel));
        if (strongMlOverride) {
            double fused = clamp01(Math.max(mlConf, (staticConf * (1.0 - blend)) + (mlConf * blend)));
            return new FusionDecision(
                    mlLabel,
                    defaultSeverityForType(mlLabel),
                    fused,
                    "ml-override"
            );
        }

        double staticVote = staticConf * (1.0 - blend);
        double mlVote = mlConf * blend;
        if (mlVote > staticVote) {
            return new FusionDecision(
                    mlLabel,
                    defaultSeverityForType(mlLabel),
                    clamp01(mlVote + (staticConf * 0.15)),
                    "blend-ml"
            );
        }
        ...
        return new FusionDecision(
                staticLabel,
                staticClassification.severity(),
                clamp01(staticVote + (mlConf * 0.15)),
                "blend-static"
        );
    }

    private static boolean isHighPriorityMlLabel(String label) {
        return EVENT_DIGITAL_VIDEO_LINK.equals(label)
                || EVENT_FHSS_CONTROL_SUSPECTED.equals(label)
                || "drone_iq_activity".equals(label);
    }

    private static String defaultSeverityForType(String type) {
        if (EVENT_DIGITAL_VIDEO_LINK.equals(type)) {
            return "WARN";
        }
        if (EVENT_FHSS_CONTROL_SUSPECTED.equals(type)) {
            return "WARN";
        }
        if ("drone_iq_activity".equals(type)) {
            return "WARN";
        }
        return "INFO";
    }
    ...
    private record FusionDecision(String type,
                                  String severity,
                                  double confidence,
                                  String mode) {
    }
